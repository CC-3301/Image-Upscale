...
- **现象**：关闭「文件添加扩展名」（引擎 `--no-rename`）且**产物路径与输入路径相同**时（同目录 + 同扩展名，NTFS 下大小写不敏感，如 `A.png` 或 `A.PNG` + `-f png`；`A.jpeg` + `-f jpg` 不算），产物就落到源文件本身。工单 50 曾加「拒绝写盘」守卫；工单 58 按维护者定案撤销守卫，改为**原地覆盖**（源图被替换、不可恢复）。
