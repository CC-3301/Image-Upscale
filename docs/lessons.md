...
- **现象**：① `realcugan-pro` + 降噪「自动」（工单 49 前该模型默认就是「自动」，现默认「高」）在带 JPEG 伪影的图上让引擎抛 `std::out_of_range` → `std::terminate`，DLL 拖垮 GUI；② 批量 AUTO 中「脏图在前、干净图在后」时，干净图沿用脏图的权重变体出图，文件名却写 `-n0`。
