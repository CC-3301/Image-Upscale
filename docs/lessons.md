...
### 2.4 降噪能力判据与持久化边界（工单 23/24/14/49/51/59/68/69）
...
- **程序化设置 `SelectedIndex` 会触发同一个 `SelectionChanged`**：`OnModelChanged` 里恢复/回退默认档时必须用抑制标志（`_suppressDenoiseWrite`）包住，否则回退值会经事件写回记忆，把用户存的档位抹掉（工单 51 曾用同款 `_suppressAddSuffixWrite`；工单 68 起后缀开关不再随输入灰置、该标志已删 —— 需抑制标志的前提是「存在程序化设值」+「事件会写回记忆」，两个开关现在都不满足）。
...
- **默认值只允许一处字面量**（工单 60）：`SettingsStore` 的表族（`DownFilterTokens` / `SwitchLabels` / `DenoiseTokens`…）配一个 `Default*` 常量，界面项与 ini 回退都引用它；`rg` 应能一眼数出字面量只有一处。（`SwitchLabels` 是工单 68 起「添加扩展名」与「删除输入文件」两个开/关开关共用的表，取代原 `AddSuffixLabels`。）
- **持久化清单**（`setting.ini`，exe 同目录，waifu2x-caffe 同构）：模型、尺寸模式 + 各模式数值、输出格式 + 质量、降采样方式（`LastDownFilter`，工单 42）、添加扩展名（`LastAddSuffix`，工单 51；界面名工单 68 去掉「文件」二字）、删除输入文件（`LastDeleteInput`，工单 69；**只认 "1" 为开**，其余值回退默认关——删除不可逆，不能把非法值当开）、降噪档位（`LastDenoise` / `LastDenoise_<modelId>`，工单 49）、窗口几何（`LastWindowLeft/Top/Width/Height/Maximized`，工单 25）；逐项校验，非法值静默回退默认。
...
### 3.15 「小问题当轮收」必须配收敛线，否则轮次无限（批次 64-66）

- **现象**：3 张纯整洁度小票（tests 夹具去重 / 引擎一行常量 / GUI 改名）在「小问题默认本轮收」的口径下跑了 **6 轮双轴评审 + 6 轮修复**：每轮只报注释措辞、命名、重复代码这类 report-only 项，且**每轮评审都会带出新的同级项**（r1 5 条 → r2 3 条 → r3 2 条 → r4 2 条 → r5 1 条 → r6 1 条），始终不到零。
- **根源**：report-only 级问题的集合不是有限的「待清列表」，而是评审的**边际产物**——只要开轮，就会产出新的措辞/品味项；把收敛条件写成「P2 清零」等于不收敛。
- **教训**：收口规则要同时钉两件事——**纳入范围**（小问题当轮提、默认收进本轮）与**收口轮次**（同一批次最多 3 轮；之后只报 P0/P1 与该轮 delta 新引入的 P2，其余记「遗留观察」）。「收到零」不是目标，「本轮定的范围内收到可合并」才是。
- **守卫**：`AGENTS.md`「评审收敛线（固定流程）」；批次汇总里列「遗留观察」清单。

### 3.16 人工验收项不能把票永远悬着（2026-09-20 维护者定案）

- **现象**：多张票（07/08/09/49/50/51/52/53/58）状态是 done，但状态行挂着「待人工验收」（GUI 视觉、启动文案、原地覆盖的实际点击效果等，只有维护者本人能跑）。没人跑 → 票永久悬空，后续批次无法判断「上一版是否真的验收过」。
- **根源**：把「人工验收」当成了关票的**前置条件**。它其实是**残余风险**，而残余风险的正确处置是「记录 + 默认接受」，不是阻塞结票。
- **教训**：人工验收项的默认处置 = **维护者无空操作时视为通过**并结清，状态行与 Comments 写清「未逐条执行的项 + 默认通过口径 + 日后提及即可重开」；复选框保持未勾，如实反映「没跑过」。日后维护者提及某项 → 重开原票或另开新票。
- **守卫**：票面 Comments 的「人工验收（默认通过）」小节；`AGENTS.md`「人工验收默认通过（固定流程）」。
