...
- **小问题不单开票、不单跑轮（固定流程）**：评审/自检报出的 report-only 级小问题（一行注释措辞、补一个测试用例、文档口径统一、重复代码清理、命名/单一定义来源之类）不要自动再跑一轮「实施 → 双轴评审 → 门禁 → 打包」，也不要一条一条单开票就当成五件活。处置：① 先把「是什么 / 影响面 / 修法 / 耗时」列给维护者，由维护者定「本轮收 / 攒批 / 不做」；② 攒批的候选票按**同一文件或同一区域**归成一批，将来实施时**一批一次闭环**（一次实施 + 一次双轴评审 + 一次门禁/打包），不按票各跑一轮；能顺手做进「正好要动那块代码」的票里更好。理由：每轮修复的固定成本（尤其打包与评审）远大于这类小问题的收益，而维护者往往宁愿先发布、把小问题留到下一版。
