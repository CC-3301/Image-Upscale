...
- **「小尾巴」构筑前先问维护者（固定流程）**：评审/自检报出的**小尾巴**（report-only 级别的小问题：
  一行注释措辞、补一个测试用例、文档口径统一之类）**不要自动再跑一轮实施 + 评审 + 门禁 + 重新打包**——
  先把「是什么、影响面、修法、大致耗时」列给维护者，由维护者决定本轮收掉还是记成候选票。
  理由：多一轮修复就要把「实施 → 双轴评审 → 门禁 → 打包」重走一遍（发布前的 dist 产物也得重做），
  代价远大于这类小问题的收益；维护者往往宁愿先发布、把尾巴留给下一版。
