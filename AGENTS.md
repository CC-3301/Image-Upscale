...
- **评审收敛线（固定流程）**：同一批次的修复/评审**最多跑到第 3 轮**；第 3 轮之后评审只报 **P0/P1** 与**由该轮 delta 新引入的 P2**，注释措辞、命名品味、重复代码、文档口径这类既有 report-only 项不再开轮——改记入票面「遗留观察」，在批次汇总里一次列给维护者。与上一条的关系：小问题仍要当轮提出、默认收进本轮，但**收口轮次**受本条约束。理由：轮次固定成本（评审 + 门禁 + 打包）远大于这类项的收益，且每轮评审都会带出新的同级项，不收线就无限循环（批次 64-66 实测：三张小票因「默认本轮收」跑了 6 轮）。
- **人工验收默认通过（固定流程）**：票面含人工验收项（真机、视觉、鼠标交互等只有维护者能跑的）时，维护者无空操作即**视为通过**：状态行与 Comments 写明「未逐条执行的项清单 + 默认通过口径 + 日后提及即可重开」，票照常结清，不因人工项悬着；复选框保持未勾，如实反映「没人跑过」。维护者日后提及某项时，重开原票或另开新票。理由：人工验收是**残余风险**（记录 + 默认接受），不是关票的前置条件（批次 07/08/09/49–53/58 实测：挂着「待人工」把票永久悬空，后续批次无法判断上一版是否真的验收过）。
