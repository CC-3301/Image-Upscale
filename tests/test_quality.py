...
from conftest import make_png, needs_engine, run_engine, MODEL
...
def test_jpg_quality_orders_file_sizes(workdir):
    ...
    low = workdir / f"A-({MODEL})-n0-2.0x-q50.jpg"
    high = workdir / f"A-({MODEL})-n0-2.0x-q95.jpg"

    # 质量段不进命名（01 命名规则无质量段），分别跑后改名收集
    p1 = run_engine(["-i", inp, "-f", "jpg", "-q", "50", "-g", "-1"])
    assert p1.returncode == 0
    out1 = workdir / f"A-({MODEL})-n0-2.0x.jpg"
    out1.rename(low)

    p2 = run_engine(["-i", inp, "-f", "jpg", "-q", "95", "-g", "-1"])
    assert p2.returncode == 0
    out2 = workdir / f"A-({MODEL})-n0-2.0x.jpg"
...
def test_png_ignores_quality(workdir):
    ...
    out = workdir / f"A-({MODEL})-n0-2.0x.png"
    assert out.exists()
...
def test_webp_quality_valid(workdir):
    ...
    assert p.returncode == 0
    out = workdir / f"A-({MODEL})-n0-2.0x.webp"
