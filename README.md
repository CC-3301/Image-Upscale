...
- 添加扩展名 开（默认）/ 关：控制产物名是否带 `-(模型名)-nN-<倍率|尺寸>`（直通缩放为 `-(Resize)-<尺寸>`）后缀段；关时产物 = 源文件同目录 + 原文件名 + 输出格式扩展名，**产物路径与输入路径相同**时（如 `A.png` 输出 PNG；扩展名不指向同一文件则不命中，`A.jpeg` 输出 JPG → 新增 `A.jpg`）则**原地覆盖源文件**（源图被替换、不可撤销）。**文件与文件夹输入都生效**：文件夹关闭时整棵树原地生成产物（不再另建带后缀的输出目录），二次运行会把上一轮产物当输入再处理一轮；同目录内同名不同扩展名的文件（`a.png` + `a.jpg`）会让产物撞车（撞到另一个输入文件、或两个产物撞到同一个路径），这种情况直接报错拒绝处理（退出码 1），不会静默盖掉源图；**开着添加扩展名时同样拒**（同目录 `1.jpg` + `1.png` 配 `-f jpg` 会算到同一个产物名），改选「与输入格式相同」就不撞（产物一个是 `.jpg`、一个是 `.png`）
- 删除输入文件 开 / 关（默认）：开启后**产物写盘成功才**删掉源文件（写盘失败则源图保留），产物与输入同路径（原地覆盖）时跳过；文件与文件夹输入都生效，文件夹逐文件删。**删除不可逆**，无备份、无二次确认
...
- 输出格式 JPG（默认质量 90）/ PNG / WebP / 与输入格式相同（逐文件跟随，产物扩展名保留输入拼写），质量 0-100 可调；除「与输入格式相同」外不跟随输入格式
