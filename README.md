...
- 文件添加扩展名 开（默认）/ 关：控制文件输入的产物名是否带 `-(模型名)-nN-<倍率|尺寸>`（直通缩放为 `-(Resize)-<尺寸>`）后缀段；关时产物 = 源文件同目录 + 原文件名 + 输出格式扩展名，**产物路径与输入路径相同**时（如 `A.png` 输出 PNG；扩展名不指向同一文件则不命中，`A.jpeg` 输出 JPG → 新增 `A.jpg`）则**原地覆盖源文件**（源图被替换、不可撤销）；文件夹输入时该开关不生效，下拉固定为开并灰置
