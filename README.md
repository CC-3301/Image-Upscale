...
- 添加扩展名 开（默认）/ 关：控制产物名是否带 `-(模型名)-nN-<倍率|尺寸>`（直通缩放为 `-(Resize)-<尺寸>`）后缀段；关时产物 = 源文件同目录 + 原文件名 + 输出格式扩展名，**产物路径与输入路径相同**时（如 `A.png` 输出 PNG；扩展名不指向同一文件则不命中，`A.jpeg` 输出 JPG → 新增 `A.jpg`）则**原地覆盖源文件**（源图被替换、不可撤销）。**文件与文件夹输入都生效**：文件夹关闭时整棵树原地生成产物（不再另建带后缀的输出目录），二次运行会把上一轮产物当输入再处理一轮；同目录内同名不同扩展名的文件（`a.png` + `a.jpg`）会让产物撞车（撞到另一个输入文件、或两个产物撞到同一个路径），这种情况直接报错拒绝处理（退出码 1），不会静默盖掉源图
