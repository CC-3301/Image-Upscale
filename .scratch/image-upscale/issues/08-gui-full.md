...
- [x] 拖入文件夹触发递归处理，展示逐文件进度与成功/失败汇总列表
- [x] 选中不支持降噪的模型（digital-art-4x、导入模型）时降噪控件禁用并提示
- [x] AUTO 档出现在降噪档位首位，手动选档后行为与 CLI 一致
- [x] GUI 与 CLI 对同一输入产出完全一致的产物（命名、格式、内容）
- [x] 手动验收清单通过
...
- **未逐条执行的项**：本票 Comments 的人工验收清单全部条目。
